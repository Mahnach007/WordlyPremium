...
class FlashcardService {
    ...
    func generateCards(
        ...
        let safeFromLanguage =
            request.fromLanguage?.trimmingCharacters(
                in: .whitespacesAndNewlines) ?? "English"
        let safeToLanguage =
            request.toLanguage?.trimmingCharacters(in: .whitespacesAndNewlines)
            ?? "Spanish"
        let safeTopic =
            request.topic?.trimmingCharacters(in: .whitespacesAndNewlines)
            ?? "Common Phrases"
        let safeCardType =
            request.cardType?.trimmingCharacters(in: .whitespacesAndNewlines)
            ?? "Mixed"
        ...
        let wordTypesString =
            safeWordTypes.isEmpty
            ? "" : "focusing on \(safeWordTypes.joined(separator: ", "))"
        ...
        let systemContent = """
            ...
            Parameter options:
            CardsAmount - Specific number of cards to generate (if not given, take 10)
        ...
        AF.request(
            apiUrl, method: .post, parameters: requestBody,
            ...
            case .success(let data):
                do {
                    ...
                    guard let choices = json?["choices"] as? [[String: Any]],
                        ...
                        print(
                            "❌ Failed to extract content string from API response"
                        )
                        let error = NSError(
                    ...
                    do {
                        ...
                        let flashcardEntities = apiResponse.flashCards.map {
                            apiCard in
                        ...
                        if flashcardEntities.isEmpty {
                            print(
                                "⚠️ Warning: API returned an empty list of flashcards"
                            )
                            let error = NSError(
                                domain: "FlashcardService",
                                code: 1002,
                        ...
                        print(
                            "Content that couldn't be parsed: \(contentString)")
    ...
    func retryRequestIfNeeded(
        _ error: Error, retryCount: Int, maxRetries: Int = 3,
